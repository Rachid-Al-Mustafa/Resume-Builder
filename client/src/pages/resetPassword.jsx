...
function ResetPassword() {
  ...
  const handleResetPassword = async (e) => {
    ...
    if (password === '' || password.length < 8) setError(true);
    setTimeout(() => {
      setError(false);
    }, 3000);

    const response = await postRequest(`/reset-password/${userId}/${token}`, {
      password,
    });
    response && navigate('/');
  };
    ...
    <div className="flex justify-center items-center h-screen bg-[#EEF5FE]">
      ...
      >
        <div className="flex flex-col gap-4">
          <div className="mx-auto w-[98px] h-[98px] bg-grayLight border-[2px] border-grayMedium rounded-full flex items-center justify-center">
            <FiLock size={50} />
          </div>
