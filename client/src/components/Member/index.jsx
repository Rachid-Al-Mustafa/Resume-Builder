...
const index = ({ member, creator, invite, communityId, online }) => {
  const { user } = useContext();
  let [isInvited, setIsInvited] = useState(false);
  ...
  const handleInvite = async () => {
    ...
    try {
      if (isInvited) {
        await postRequest(
          `/community/cancel-community-invite-request/${communityId}/${_id}`
        );
      } else {
        await postRequest(
          `/community/send-community-invite-request/${communityId}/${_id}`
        );
      }
      setLoading(false);
    } catch (error) {
  ...
  return (
    <div
      className={`flex items-center justify-between gap-2 ${
        !actualUser ? 'cursor-pointer' : 'cursor-auto'
      }`}
    >
      <Link
      ...
      >
        <div className="relative">
          ...
          />{' '}
          {online && (
            <span className="absolute w-[15px] h-[15px] bg-[limegreen] rounded-full top-0 right-0 border-2 border-white"></span>
          )}
        </div>
